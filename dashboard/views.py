...
class Client(View):
    ...
    def post(self, request):
        print(request.POST.values, 'gfdgdfgdfgdfgfd')
        if ClientTable.objects.filter(phone=request.POST.get("phone")).exists():
            ...
            return render(
                request,
                "client.html",
                {"alternate_error": "Alternate Phone is not valid"},
            )
...
class update(View):
    ...
    def get(self, request, id):
        ...
        print(type(client.theropy),"FHF")
        assesment_client.append(str(client.assessment))
        
        return render(request, "edit-client.html", {"client": client,"assesment_client":assesment_client,"theropy":client.theropy})

    def post(self, request, id):
        print(request.POST.values, 'gfdgdfgdfgdfgfd')
        print("postttt", request.POST.get("phone"))
        thero = []
        # print("therpy", request.POST.getlist("theropy"))
        client = ClientTable.objects.get(id=id,user=request.user)

        if not re.fullmatch(regex, request.POST.get("email")):
            print("SHOW")
            return render(request, 'edit-client.html',{'email_error':'Email is not valid'})
        elif not re.fullmatch(regex2, request.POST.get('name').strip()):
            return render(request, 'edit-client.html',{'name_error':'Only Characters are required'  })
        elif not re.fullmatch(regex2, request.POST.get('mother_tongue').strip()):
            return render(request, 'edit-client.html',{'mother_tongue':'Only Characters are required'})
        elif client.phone != request.POST.get("phone"):
            for cli in client.theropy:
                thero.append(cli)
                print(thero)
            print(client.theropy,"ruunnn")
            val = re.fullmatch(phone_regex, request.POST.get("phone"))
            if val is None:
                return render(
                    request, "edit-client.html", {"phone_error": "Phone is not valid", 'thero':thero,  'update_by':request.POST.getlist("assessment")}
                )
        # elif client.theropy != request.POST.get("theropy"):
        #     val = re.fullmatch(theropy_regex, request.POST.get("theropy"))
        #     if val is None:
        #         return render(
        #             request, "edit-client.html", {"phone_error": "Phone is not valid", 'update_by':request.POST.getlist("theropy")}
        #         )        
            ...
            if val is None:
                return render(
                    request,
                    "edit-client.html",
                    {"alternate_phone": "Alternate Phone is not valid"},
                )
...
class UserListing(View):
    ...
    def get(self, request):
        ...
        #     print("de", request.GET.get('department'))
        if department:
            clients = User.objects.filter(department=request.GET.get('department'), created_by=request.user)
            print(clients, 'clients vcheckj')
            page = request.GET.get("page", 1)
        ...
        if search:
            ...
            clients = User.objects.filter(
                ...
                | Q(department__icontains=request.GET.get("search")),user=request.user
            )
            page = request.GET.get("page", 1)
    ...
    def get(self, request):
        ...
        if request.GET.get("search") and request.GET.get("date"):
            ...
            try:
                ...
                client = paginator.page(paginator.num_pages)
            html = render_to_string("listing-assessment-filter.html", {"client":client, "blnk_dic":blnk_dic})
            return JsonResponse({"html": html})

            ## ### kishan code
        if request.GET.get("date") and not request.GET.get("search"):
            print("both valaa", request.GET.get("date"))
            blnk_dic = {}
            status = {}
            clients = ClientTable.objects.filter(created_on=request.GET.get("date") ,user=request.user)
            for k in clients:
            ...
            html = render_to_string("listing-assessment-filter.html", {"client": client,"blnk_dic":blnk_dic,"status":status})
            return JsonResponse({"html": html})
        ...
        if request.GET.get("search") and not request.GET.get("date"):
            ...
            clients = ClientTable.objects.filter(
                ...
                | Q(name__icontains=request.GET.get("search"),user=request.user)
            )
            # newly added
            for s in clients:
                blnk_dic[s.id] = s.assessment
